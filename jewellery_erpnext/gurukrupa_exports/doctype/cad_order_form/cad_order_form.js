frappe.ui.form.on("CAD Order Form", {
	...
	setup: function (frm, cdt, cdn) {
		...
		let design_fields = [
			["design_id", "tag_no"],
			["reference_designid", "reference_serial_no_1"],
			["reference_design_id_2", "reference_serial_no_2"],
			["reference_design_id_3", "reference_serial_no_3"],
		];
...
function set_metal_properties_from_bom(frm, cdt, cdn) {
	...
	if (row.design_type != "Mod" && (row.serial_no_bom || row.bom)) {
		frappe.db.get_value(
			...
			["metal_touch", "metal_type", "metal_colour"],
			(r) => {
			...
			if (subcategory_attribute_value.is_subcategory == 1) {
				if (subcategory_attribute_value.item_attributes) {
					$.each(subcategory_attribute_value.item_attributes, function (index, row) {
						if (row.in_cad == 1) show_field(frm, cdt, cdn, row.item_attribute);
					});
				}
			}
		});
