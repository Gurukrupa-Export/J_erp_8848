...
let set_si_reference_field_filter = () =>{
    cur_frm.set_query("si_reference_field", function() {
        if (cur_frm.doc.purchase_type === 'FG Purchase'){
            return {
                'filters':{
                    'sales_type': 'FG Sales'
                }
...
let get_items = (frm) =>{
    ...
    frm.add_custom_button(__('Sales Invoice'), function () {
        let query_args = {
            filters: { docstatus: ["!=", 2], sales_type: cur_frm.doc.purchase_type === 'FG Purchase'? 'FG Sales': 'Branch Sales' }
        }
...
let get_purchase_type = (frm) => {
    ...
    if (frm.doc.supplier){
        frappe.call({
            ...
            freeze: true,
            args: {
                doc: 'Purchase Type', parent: frm.doc.supplier, field: 'purchase_type'
            },
            callback: function (r) {
...
let change_df = (field) => {
    var df = frappe.meta.get_docfield("Purchase Receipt Item", field, cur_frm.doc.name)
    df.read_only = 1;
}